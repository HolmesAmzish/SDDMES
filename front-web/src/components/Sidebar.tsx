...
export default function Sidebar({ title }: SidebarProps) {
  ...
  const groups = [
    {
      title: "生产管理",
      items: [
        { label: "生产信息", path: "/notification", icon: <FaIndustry /> },
  ...
  return (
    <aside className="sticky top-0 w-64 bg-gradient-to-br from-blue-400 to-blue-800 h-screen p-4 flex flex-col overflow-y-auto border-r border-gray-200">
            ...
            {group.items.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-2 p-2 rounded hover:bg-blue-400 text-white ${
